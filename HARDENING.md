...
- Restart incomplete uploads after upgrading; staged overwrites use additional Redis upload metadata. Resumable overwrites now write to a private `.filebrowser-upload-*` sibling and replace the original only after the upload is synced and closed. Cancellation removes the staged file. New resumable uploads still keep partial data at their destination. Abandoned staging files are retained, like other abandoned uploads.
- Ordinary uploads and editor saves stage, sync, and close their data before renaming it over the destination. Read, write, sync, close, or rename failures leave the original intact. Before-hook failures leave it untouched; after-hook failures report an error but keep the completed replacement. Destination revision checks detect intervening changes before replacement; they are not an atomic compare-and-swap and do not eliminate races with host filesystem changes or other APIs.
- Replacement preserves existing permission bits but creates a new inode owned by the server process; ownership, ACLs, extended attributes, and hard-link relationships are not preserved. Atomic visibility depends on the filesystem's rename guarantees. Writes through in-scope symbolic links update their resolved target; a final symbolic link that still resolves outside scope is rejected. Staging requires free space for both the old and new content and write access to the containing directory.
...
User-update fields are normalized and validated before authorization. Full updates (including an omitted or empty `which` list), password-lock changes, and rule changes require the acting user's current password under JSON authentication. Preferences remain editable without reauthentication. CLI user updates preserve unspecified password-lock and display flags, including the editor theme, and accept explicit false values.

CI runs frontend lint, type checking and tests, backend lint and race tests, a real Redis integration service, and the combined build. Every test job is a release prerequisite; the workflow also supports manual runs. Image publishing remains a separate release step using the existing release configuration.
