...
- Password login, signup, and password-protected share checks share a limit of 20 attempts per minute per transport peer, with four concurrent password operations. A reverse proxy's clients share its transport-peer limit; forwarded client-IP headers are intentionally not trusted. Configure additional limits at a trusted proxy for larger installations.
